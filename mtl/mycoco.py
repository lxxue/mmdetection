...
class MyCocoDataset(CocoDataset):
    ...
    def prepare_train_img(self, idx):
        ...
        if self.with_cap:
            ...
            data['cap'] = DC(to_tensor(self.enc_captions[rnd_idx]))
            data['caplen'] = DC(to_tensor(self.caplens[rnd_idx]))
        return data
    ...
    def prepare_test_img(self, idx):
        ...
        if self.with_cap is not None:
            data['caps'] = self.enc_captions[idx*self.cpi:(idx+1)*self.cpi]
            data['caplens'] = self.caplens[idx*self.cpi:(idx+1)*self.cpi]
        return data
